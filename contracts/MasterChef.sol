...
// will be transferred to a governance smart contract once AVALab is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accAVALabPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAVALabPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accAVALabPerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }

    // The AVALab TOKEN!
    AVALabToken public AVALab;
    // Dev address.
    ...
    // Block number when bonus AVALab period ends.
    uint256 public bonusEndBlock;
    // AVALab tokens created per block.
    uint256 public AVALabPerBlock;
    //Amount of sushis per Second
    uint256 public AVALabPerSecond;
    // Bonus muliplier for early AVALab makers.
    ...
    // The block number when AVALab mining starts.
    ...
    event RewardChange(uint256 AVALabPerBlock, uint256 Blocktime);
   ...
   modifier validPID(uint256 _pid){
    ...
    constructor(
        ...
        AVALabPerBlock = _sushiPerBlock;
        AVALabPerSecond = _sushiPerSecond;
    ...
    function updateBlockIssuanceWithBlocktime()
        ...
        AVALabPerBlock = currentBlocktime.mul(AVALabPerSecond).div(1000);
        lastBlocktime = currentBlocktime;
        emit RewardChange(AVALabPerBlock, currentBlocktime);
    }

    //Changes the Blocktime Parameter and updates the Rewards per Block
    function changeBlocktime(uint _currentBlocktime)
        public onlyOwner {
        require(lastBlocktime != _currentBlocktime , "AVALab : Blocktime hasn't changed");
    ...
    // Update the given pool's AVALab allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function pendingAVALab(uint256 _pid, address _user)
        external
        view
        validPID(_pid)
        returns (uint256)
        ...
        uint256 accAVALabPerShare = pool.accAVALabPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 AVALabReward =
                multiplier.mul(AVALabPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accAVALabPerShare = accAVALabPerShare.add(
                AVALabReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accAVALabPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public validPID(_pid) {
        ...
        AVALab.mint(devaddr, AVALabReward.div(devDivisor));
        AVALab.mint(address(this), AVALabReward);
        pool.accAVALabPerShare = pool.accAVALabPerShare.add(
            AVALabReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for AVALab allocation.
    function deposit(uint256 _pid, uint256 _amount) public validPID(_pid) nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAVALabPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAVALabPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validPID(_pid) nonReentrant{
        ...
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accAVALabPerShare).div(1e12).sub(
                user.rewardDebt
            );
        safeTokenTransfer(msg.sender, pending);
    ...
    // Safe AVALab transfer function, just in case if rounding error causes pool to not have enough SUSHIs.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = AVALab.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBal) {
            transferSuccess = AVALab.transfer(_to, tokenBal);
        } else {
            transferSuccess = AVALab.transfer(_to, _amount);
        }
