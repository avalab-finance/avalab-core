...
contract SushiMaker is Ownable {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(token != sushi && token != weth && token != bridge, "AVALab Maker: Invalid bridge");
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "AVALab Maker: must use EOA");
        _;
    }
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "AVALab Maker: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
    ...
    function _swap(address fromToken, address toToken, uint256 amountIn, address to) internal returns (uint256 amountOut) {
        ...
        require(address(pair) != address(0), "AVALab Maker: Cannot convert");
